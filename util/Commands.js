module.exports = (Discord, client, config) => {
  ...
  require('fs').readdir('./commands', (err, files) => {
      if (files) {
          ...
          console.log(`[COMMANDS] ${files.length} adet komut bulundu, yükleniyor...`);
          files.forEach(file => {
              loadedCommands++
              let command = require(`${process.cwd()}/${'./commands'}/${file}`);

              function kill() {
                throw new Error('[ERROR] Komut dosyası yapısında hata mevcut!');
              };
              ...
              if (loadedCommands == files.length) console.log(`[COMMANDS] Komutlar başarıyla yüklendi! (${files.length})`);
          });
      };
  });
};